...
def leven_dist_raw(log1, log2, percent_1, percent_2):
    '''
    This function compares the raw Levenshtein distance between two sublogs without normalization.
    '''
    ...
    if var_list_1 == var_list_2:
        ...
        for i in range(max_len):
            for j in range(min_len):
                (str1, str2) = leven_preprocess(max_var[i], min_var[j])
                dist_matrix[i][j] = string_distance.levenshtein(str1, str2)

        dist = np.sum(np.min(dist_matrix, axis=1))
